"""
Словарь сообщений сервера на двух языках — только наши собственные, вручную
написанные сообщения (server.py, param_options.py, report_columns.py).

Сырой текст исключений/ошибок БД (traceback в worker.py, ошибки Postgres) сюда
не входит и не переводится — та часть зависит от языка самой БД, не от нас.

Язык берётся из .env (LANGUAGE=ru/en) — настройка разворачивания, не пользователя.
"""
import os
